...
function tequilarebase() {
    ...
    git fetch "https://review.tequilaos.pl/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
...
function tequilaremote()
{
    ...
    if [ -z "$TEQUILA_USER" ]
    then
        git remote add tequila ssh://review.tequilaos.pl:29418/$PFX$PROJECT
    else
        git remote add tequila ssh://$TEQUILA_USER@review.tequilaos.pl:29418/$PFX$PROJECT
    fi
